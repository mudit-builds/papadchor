...
def u_query_search(all_inc):
    """Search for SLA records by SR numbers with updated values"""
    try:
        df = update_sql("""select "SR","INC Number", "Created"
                ,"Resolved_Time","Network","Product","Acc Name" ,"Acc Num", "Circ",
                "OLO Tier", "Resilience" , MRC, CURRENCY,"SLA%","SLA Amnt",
...
def read_data(form_data):
    """Process form data and update database with recalculated values"""
        ...
        update_sql(f"""
            UPDATE OI_RTQM_L1.ID1558_SLA_COMPENSATION_CALCULATOR
               SET
                ...
                U_CURRENCY = '{row['CURRENCY'].replace('None', '')}',
...
def search():
    ...
    return render_template("input_value.html", form=form, records=records, columns=columns, sr_num=sr_num)
...
def recalculate():
    ...
    return render_template("input_value.html", all_inc=str(all_inc), form=form, form1=form1,
                          ...
                          records1=records_re, columns1=columns_re)
...
def download_pdf():
    """Generate and download PDF reports as a ZIP file"""
